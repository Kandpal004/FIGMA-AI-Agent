...
minting a :class:`WFEvidence` per distinct signal and de-duplicating on ``(provenance,
external_ref, normalised claim)`` so the same fact surfaced by two sources is not
double-counted. The resulting graph is what the planner cites from and what the plan
validates against.
"""
...
from wireframe.domain.shared.value_objects import Confidence, Tag
