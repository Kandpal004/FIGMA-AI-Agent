...
These implement the nine input ports with pre-supplied :class:`RawSignal` s, so the engine
runs and is tested with no upstream engines wired. They are the honest default for local
running and tests (a stand-in for the real Phase 3–11 adapters) and demonstrate the port
contracts in the simplest possible form. A null adapter returns nothing — valid when a source
is unavailable.
"""
