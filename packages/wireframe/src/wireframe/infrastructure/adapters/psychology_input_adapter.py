"""PsychologyInputAdapter — feeds Phase-9 Customer Psychology into the wireframe plan.
...
and trust needs into :class:`RawSignal` s (provenance ``PSYCHOLOGY``), so trust blocks and
section placement are grounded in how the customer decides. The wireframe domain never imports
Phase 9, so this adapter is the seam.
"""
        ...
        for stage in bundle.journey_stages:
            ...
            signals.append(
                RawSignal(
                    ...
                    tags=(stage["phase"], "trust", "review", "confidence", "conversion"),
                )
            )
        for obj in bundle.objections:
            signals.append(
                RawSignal(
                    ...
                    tags=(obj["phase"], "objection", "trust", "guarantee"),
                )
            )
        return signals
