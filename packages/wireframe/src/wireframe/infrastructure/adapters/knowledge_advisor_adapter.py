"""KnowledgeAdvisorAdapter — grounds the wireframe plan in the Phase-3 Knowledge Engine.

Implements :class:`KnowledgeAdvisorPort` over the Phase-3 query service: for each planning
topic (section ordering, component composition, approval gating, accessibility, performance,
…) it searches the curated corpus and returns the matching principles as neutral
:class:`RawSignal` s (provenance ``KNOWLEDGE``), de-duplicated by lineage. The wireframe
            ...
            for entry in result.entries:
                ...
                signals.append(
                    RawSignal(
                        ...
                        tags=(entry.category.value, "component", "pattern", "structure"),
                    )
                )
        return signals
