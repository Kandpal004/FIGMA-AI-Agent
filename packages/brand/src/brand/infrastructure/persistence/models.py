"""SQLAlchemy ORM model — the durable shape of a produced brand strategy report.

A report is stored as one row: the full aggregate as a JSON ``document`` (via the codec)
plus indexed, queryable columns (lineage, version, project, primary category, archetype,
...
__all__ = ["Base", "BrandReportModel"]


class Base(DeclarativeBase):
    """Declarative base for the Brand Strategy ORM models."""


class BrandReportModel(Base):
    """One produced brand strategy report version."""
