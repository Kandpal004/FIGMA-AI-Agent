...
:class:`RawSignal` s (provenance ``INFORMATION_ARCHITECTURE``), so page coverage and the RTL
mirror rule are grounded. The design-system domain never imports Phase 11, so this adapter is the
seam.
"""
    ...
    async def gather(self, project: ProjectContext) -> Sequence[RawSignal]:
        ...
        signals: list[RawSignal] = [
            RawSignal(
                ...
                claim="The information architecture defines the pages the design system must "
                "cover and the reading direction it must support (LTR/RTL).",
                confidence=0.85,
                source_name="Information Architecture",
                tags=("structure", "page", "navigation", "rtl", "direction"),
            )
        ]
        for page in bundle.pages:
            signals.append(
                RawSignal(
                    ...
                    confidence=0.8,
                    source_name="Information Architecture",
                    tags=(page["page_type"], "structure", "page", "navigation"),
                )
            )
        return signals
