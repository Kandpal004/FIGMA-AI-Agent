...
Implements :class:`KnowledgeAdvisorPort` over the Phase-3 query service: for each design-system
topic it searches the curated corpus and returns the matching principles as neutral
:class:`RawSignal` s (provenance ``KNOWLEDGE``), de-duplicated by lineage. The design-system
domain never imports Phase 3; this adapter is the seam.
"""
            ...
            for entry in result.entries:
                ...
                signals.append(
                    RawSignal(
                        ...
                        tags=(entry.category.value, "token", "theme", "accessibility",
                              "design-system"),
                    )
                )
        return signals
