...
directive bundle into :class:`RawSignal` s (provenance ``BUSINESS_STRATEGY``), so platform
priority and the performance constraint are grounded. The design-system domain never imports
Phase 7, so this adapter is the seam.
"""
    ...
    async def gather(self, project: ProjectContext) -> Sequence[RawSignal]:
        ...
        return [
            RawSignal(
                ...
                tags=(bundle.tier, "business", "positioning", "performance", "platform"),
            ),
            RawSignal(
                provenance=ProvenanceKind.BUSINESS_STRATEGY,
                external_ref=f"{ref}:performance",
                claim="The design system must protect conversion through a performance budget on "
                "every component.",
                confidence=0.85,
                source_name="Business Strategy",
                tags=("business", "performance", "conversion", "budget"),
            ),
        ]
