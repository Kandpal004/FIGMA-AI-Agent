"""Integration test: the design execution plan grounded in the REAL Phase-16 Design System,
Phase-15 Component Intelligence, Phase-14 Design Language, Phase-13 Creative Director, Phase-12
Wireframe, Phase-11 IA, Phase-10 UX, Phase-9 Psychology, Phase-8 Brand, Phase-7 Business
Strategy, and Phase-3 Knowledge engines — no fakes, the whole upstream stack, live."""
...
# --- Phase 17: Design Orchestrator Engine (under test) ---------------------
...
from design_orchestrator.domain.context.context import (
    OrchestrationBrief, ProjectContext, SourceRefs,
)
from design_orchestrator.domain.shared.ids import DesignExecutionPlanId, DONodeId
from design_orchestrator.domain.shared.value_objects import GraphKind
from design_orchestrator.infrastructure.adapters.brand_input_adapter import BrandInputAdapter
from design_orchestrator.infrastructure.adapters.business_strategy_input_adapter import BusinessStrategyInputAdapter
from design_orchestrator.infrastructure.adapters.component_intelligence_input_adapter import ComponentIntelligenceInputAdapter
from design_orchestrator.infrastructure.adapters.creative_director_input_adapter import CreativeDirectorInputAdapter
from design_orchestrator.infrastructure.adapters.design_language_input_adapter import DesignLanguageInputAdapter
from design_orchestrator.infrastructure.adapters.design_system_input_adapter import DesignSystemInputAdapter
from design_orchestrator.infrastructure.adapters.ia_input_adapter import IAInputAdapter
from design_orchestrator.infrastructure.adapters.knowledge_advisor_adapter import KnowledgeAdvisorAdapter
from design_orchestrator.infrastructure.adapters.psychology_input_adapter import PsychologyInputAdapter
from design_orchestrator.infrastructure.adapters.ux_input_adapter import UXInputAdapter
from design_orchestrator.infrastructure.adapters.wireframe_input_adapter import WireframeInputAdapter
...
async def test_execution_plan_grounded_in_live_upstream():
    kenv = build_knowledge()
    await _seed_knowledge(kenv, "Section ordering and above-the-fold sequencing",
                          "Hero above the fold, trust before the conversion ask, and reviews and "
                          "related products deferred below the fold maximise premium conversion.")
    ...
    # Phase 17: orchestrate the execution plan over the REAL upstream adapters.
    env = build_do(
    ...
    view = await env.facade.orchestrate(BuildExecutionPlan(request=OrchestrationRequest(
        ...
        source_refs=SourceRefs(design_system_spec_id=str(ds[1]), component_spec_id=str(ci[1]),
    ...
    assert view.execution_order[0]["label"] == "setup_theme"
    ...
    for kind in GraphKind:
        for node in view.graphs[kind.value]["nodes"]:
            trace = await env.facade.explain(pid, kind, DONodeId.from_string(node["id"]))
