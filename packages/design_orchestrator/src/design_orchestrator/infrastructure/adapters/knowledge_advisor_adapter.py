...
Implements :class:`KnowledgeAdvisorPort` over the Phase-3 query service: for each orchestration
topic it searches the curated corpus and returns the matching principles as neutral
:class:`RawSignal` s (provenance ``KNOWLEDGE``), de-duplicated by lineage. The orchestrator domain
            ...
            for entry in result.entries:
                ...
                signals.append(
                    RawSignal(
                        ...
                        tags=(entry.category.value, "order", "sequence", "orchestration"),
                    )
                )
        return signals
