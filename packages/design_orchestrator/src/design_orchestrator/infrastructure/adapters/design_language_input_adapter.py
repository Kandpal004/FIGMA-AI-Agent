...
into :class:`RawSignal` s (provenance ``DESIGN_LANGUAGE``), so the visual and typography choices
are grounded in the language. The orchestrator domain never imports Phase 14, so this adapter is
the seam.
"""
    ...
    async def gather(self, project: ProjectContext) -> Sequence[RawSignal]:
        ...
        return [
            RawSignal(
                ...
                claim=f"Visual DNA: {view.visual_dna.get('essence', '')} — the section visual and "
                "typography choices express this language.",
                confidence=0.85,
                source_name="Design Language",
                tags=("visual", "typography", "emphasis", "language",
