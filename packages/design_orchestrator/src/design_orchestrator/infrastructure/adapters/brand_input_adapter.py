...
bundle into :class:`RawSignal` s (provenance ``BRAND_STRATEGY``), so the visual emphasis choices
are grounded in the brand. The orchestrator domain never imports Phase 8, so this adapter is the
seam.
"""
    ...
    async def gather(self, project: ProjectContext) -> Sequence[RawSignal]:
        ...
        return [
            RawSignal(
                ...
                claim=f"Brand tone {bundle.tone} shapes the emphasis and theme of hero and "
                "conversion sections.",
                confidence=0.8,
                source_name="Brand Strategy",
                tags=(bundle.archetype, bundle.tone, "brand", "emphasis", "visual"),
            )
        ]
