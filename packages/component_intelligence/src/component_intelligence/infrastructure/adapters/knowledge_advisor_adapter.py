"""KnowledgeAdvisorAdapter — grounds the engine in the Phase-3 Knowledge Engine.

Implements :class:`KnowledgeAdvisorPort` over the Phase-3 query service: for each
component-intelligence topic it searches the curated corpus and returns the matching principles
as neutral :class:`RawSignal` s (provenance ``KNOWLEDGE``), de-duplicated by lineage. The
component-intelligence domain never imports Phase 3; this adapter is the seam.
"""
        ...
        for topic in topics:
            ...
            for entry in result.entries:
                ...
                signals.append(RawSignal(
                    provenance=ProvenanceKind.KNOWLEDGE, external_ref=key, claim=entry.statement,
                    confidence=entry.confidence.score, statement=entry.title,
                    source_name="Knowledge Engine",
                    tags=(entry.category.value, "component", "pattern", "composition", "compatibility")))
        return signals
