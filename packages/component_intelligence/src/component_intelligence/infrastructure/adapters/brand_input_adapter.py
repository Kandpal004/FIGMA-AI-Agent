...
bundle into :class:`RawSignal` s (provenance ``BRAND_STRATEGY``). The component-intelligence
domain never imports Phase 8, so this adapter is the seam.
"""
...
class BrandInputAdapter:
    ...
    async def gather(self, project: ProjectContext) -> Sequence[RawSignal]:
        ...
        return [RawSignal(
            ...
            claim=f"Brand tone {bundle.tone} shapes brand-expressive components.",
            confidence=0.8, source_name="Brand Strategy",
            tags=(bundle.archetype, bundle.tone, "brand", "identity", "trust"))]
