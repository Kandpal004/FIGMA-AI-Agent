...
These implement the eleven input ports with pre-supplied :class:`RawSignal` s, so the engine
runs and is tested with no upstream engines wired. They are the honest default for local
running and tests (a stand-in for the real Phase 3–14 adapters). A null adapter returns nothing.
"""
...
InMemoryResearchInput = _gather_class("InMemoryResearchInput")
InMemoryCompetitorInsight = _gather_class("InMemoryCompetitorInsight")
...
NullResearchInput = _null_class("NullResearchInput")
NullCompetitorInsight = _null_class("NullCompetitorInsight")
