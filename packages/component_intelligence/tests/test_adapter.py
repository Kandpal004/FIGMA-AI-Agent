"""Integration test: the component composition grounded in the REAL Phase-14 Design Language,
Phase-13 Creative Director, Phase-12 Wireframe, Phase-11 IA, Phase-10 UX, Phase-9 Psychology,
Phase-8 Brand, Phase-7 Business Strategy, and Phase-3 Knowledge engines — no fakes, the whole
upstream stack, live."""
...
# --- Phase 15: Component Intelligence Engine (under test) ------------------
...
from component_intelligence.domain.shared.ids import CINodeId, ComponentSpecId
from component_intelligence.domain.shared.value_objects import GraphKind
from component_intelligence.infrastructure.adapters.brand_input_adapter import BrandInputAdapter
from component_intelligence.infrastructure.adapters.business_strategy_input_adapter import BusinessStrategyInputAdapter
from component_intelligence.infrastructure.adapters.creative_director_input_adapter import CreativeDirectorInputAdapter
from component_intelligence.infrastructure.adapters.design_language_input_adapter import DesignLanguageInputAdapter
from component_intelligence.infrastructure.adapters.ia_input_adapter import IAInputAdapter
from component_intelligence.infrastructure.adapters.knowledge_advisor_adapter import KnowledgeAdvisorAdapter
from component_intelligence.infrastructure.adapters.psychology_input_adapter import PsychologyInputAdapter
from component_intelligence.infrastructure.adapters.ux_input_adapter import UXInputAdapter
from component_intelligence.infrastructure.adapters.wireframe_input_adapter import WireframeInputAdapter
...
async def test_composition_grounded_in_live_upstream():
    kenv = build_knowledge()
    await _seed_knowledge(kenv, "Component patterns for conversion",
                          "Trust badges, reviews, sticky add-to-cart and clear composition lift "
    ...
    # Phase 15: compose components over the REAL upstream adapters.
    env = build_ci(
    ...
    for kind in GraphKind:
        for node in view.graphs[kind.value]["nodes"]:
            trace = await env.facade.explain(sid, kind, CINodeId.from_string(node["id"]))
