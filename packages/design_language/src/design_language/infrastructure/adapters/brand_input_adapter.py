"""BrandInputAdapter — feeds the Phase-8 Brand Strategy into the language (its primary driver).
...
s (provenance ``BRAND_STRATEGY``), so the Visual DNA and personalities are grounded in the
brand identity. The design-language domain never imports Phase 8, so this adapter is the seam.
"""
    ...
    async def gather(self, project: ProjectContext) -> Sequence[RawSignal]:
        ...
        return [
            RawSignal(
                ...
                tags=(bundle.archetype, "brand", "identity", "positioning", "tone"),
            ),
            RawSignal(
                provenance=ProvenanceKind.BRAND_STRATEGY, external_ref=f"{ref}:tone",
                claim=f"Brand tone {bundle.tone} sets the visual voice and personality.",
                confidence=0.85, source_name="Brand Strategy",
                tags=(bundle.tone, "brand", "tone", "voice", "identity"),
            ),
        ]
