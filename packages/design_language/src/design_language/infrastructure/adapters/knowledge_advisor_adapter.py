"""KnowledgeAdvisorAdapter — grounds the language in the Phase-3 Knowledge Engine.

Implements :class:`KnowledgeAdvisorPort` over the Phase-3 query service: for each visual-system
topic (premium visual systems, typographic scale, spacing rhythm, restraint, timelessness, …)
it searches the curated corpus and returns the matching principles as neutral :class:`RawSignal`
s (provenance ``KNOWLEDGE``), de-duplicated by lineage, so the tokens and philosophies are
grounded in canonical best-practice. The design-language domain never imports Phase 3; this
adapter is the seam.
"""
            ...
            for entry in result.entries:
                ...
                signals.append(
                    RawSignal(
                        ...
                        tags=(entry.category.value, "spacing", "type", "grid", "system",
                              "restraint", "premium", "timeless"),
                    )
                )
        return signals
