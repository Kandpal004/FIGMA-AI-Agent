"""PsychologyInputAdapter — feeds Phase-9 Customer Psychology into the IA.
...
and trust needs into :class:`RawSignal` s (provenance ``PSYCHOLOGY``), so section placement and
trust structure are grounded in how the customer decides. The adapter is bound to a resolved
psychology report; the IA domain never imports Phase 9, so this adapter is the seam.
"""
        ...
        for stage in bundle.journey_stages:
            ...
            signals.append(
                RawSignal(
                    ...
                    claim=f"At {stage['phase']}: emotion {stage.get('emotion', '')}, trust needed: {trust}.",
                    confidence=0.85, source_name="Customer Psychology",
                    tags=(stage["phase"], "trust", "review", "conversion"),
                )
            )
        for obj in bundle.objections:
            signals.append(
                RawSignal(
                    ...
                    claim=f"Objection at {obj['phase']}: {obj['objection']} — resolve by {obj['resolution']}.",
                    confidence=0.8, source_name="Customer Psychology",
                    tags=(obj["phase"], "objection", "trust", "product"),
                )
            )
        return signals
