"""BrandInputAdapter — feeds Phase-8 Brand Strategy into the IA.
...
s (provenance ``BRAND_STRATEGY``), so structural emphasis (which pages and sections carry the
brand's voice, where trust is expressed) is grounded in the brand identity. The adapter is
bound to a resolved brand report; the IA domain never imports Phase 8, so this adapter is the
seam.
"""
