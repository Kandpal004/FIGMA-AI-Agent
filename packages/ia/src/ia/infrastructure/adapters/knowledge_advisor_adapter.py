"""KnowledgeAdvisorAdapter — grounds the information architecture in the Phase-3 Knowledge Engine.

Implements :class:`KnowledgeAdvisorPort` over the Phase-3 query service: for each IA topic
(navigation, taxonomy, faceted search, breadcrumbs, internal linking, …) it searches the
curated corpus and returns the matching principles as neutral :class:`RawSignal` s
(provenance ``KNOWLEDGE``), de-duplicated by lineage. The IA domain never imports Phase 3;
