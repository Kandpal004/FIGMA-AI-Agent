...
minting an :class:`IAEvidence` per distinct signal and de-duplicating on ``(provenance,
external_ref, normalised claim)`` so the same fact surfaced by two sources is not
double-counted. The resulting graph is what the architect cites from and what the report
validates against.
"""
...
from ia.domain.shared.value_objects import Confidence, Tag
