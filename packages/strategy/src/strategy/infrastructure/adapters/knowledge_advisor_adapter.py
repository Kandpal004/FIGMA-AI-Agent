"""KnowledgeAdvisorAdapter — grounds strategy in the Phase-3 Knowledge Engine.

Implements :class:`KnowledgeAdvisorPort` over the Phase-3 query service: for each
strategic topic it searches the curated corpus and returns the matching principles as
neutral :class:`RawInsight` s (provenance ``KNOWLEDGE``), de-duplicated by lineage. The
strategy domain never imports Phase 3; this adapter is the seam.
"""
    ...
    async def advise(
        ...
        for topic in topics:
            result = await self._query.search(
                topic,
                query=KnowledgeQuery(viewer_tenant_id=viewer, limit=_PER_TOPIC),
            )
