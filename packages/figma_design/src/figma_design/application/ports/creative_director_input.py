"""The Phase 13 Creative Director port — grounds the quality bar and dev-mode notes.

Supplies neutral :class:`RawSignal` s derived from the approved review (the quality gates and
handoff expectations). The infrastructure adapter imports that engine and translates; the
figma-design domain never imports it — nor any Figma SDK.
"""

from __future__ import annotations

from collections.abc import Sequence
from typing import Protocol, runtime_checkable

from figma_design.application.contracts import RawSignal
from figma_design.domain.context.context import ProjectContext

__all__ = ["CreativeDirectorInputPort"]


@runtime_checkable
class CreativeDirectorInputPort(Protocol):
    """Gathers creative-director signals as neutral signals."""

    async def gather(self, project: ProjectContext) -> Sequence[RawSignal]:
        """Return signals for a project (may be empty)."""
        ...
