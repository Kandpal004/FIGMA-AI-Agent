...
Implements :class:`KnowledgeAdvisorPort` over the Phase-3 query service: for each file-craft topic
it searches the curated corpus and returns the matching principles as neutral :class:`RawSignal` s
(provenance ``KNOWLEDGE``), de-duplicated by lineage. The figma-design domain never imports Phase 3
— nor any Figma SDK — so this adapter is the seam.
"""
