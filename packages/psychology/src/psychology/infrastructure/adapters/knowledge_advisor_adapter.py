"""KnowledgeAdvisorAdapter — grounds the psychology in the Phase-3 Knowledge Engine.

Implements :class:`KnowledgeAdvisorPort` over the Phase-3 query service: for each
behavioral topic it searches the curated corpus and returns the matching principles as
neutral :class:`RawSignal` s (provenance ``KNOWLEDGE``), de-duplicated by lineage. The
psychology domain never imports Phase 3; this adapter is the seam.
"""
