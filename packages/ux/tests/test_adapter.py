"""Integration test: UX strategy grounded in the REAL Phase-9 Psychology, Phase-8 Brand,
Phase-7 Business Strategy, and Phase-3 Knowledge engines — no fakes, live upstream."""
...
from psychology.domain.context.context import (
    ProjectContext as PsyProject,
    PsychologyBrief,
)
...
from ux.domain.shared.ids import UXNodeId, UXReportId
from ux.domain.shared.value_objects import GraphKind
from ux.infrastructure.adapters.brand_input_adapter import BrandInputAdapter
from ux.infrastructure.adapters.business_strategy_input_adapter import (
    BusinessStrategyInputAdapter,
)
from ux.infrastructure.adapters.knowledge_advisor_adapter import KnowledgeAdvisorAdapter
from ux.infrastructure.adapters.psychology_input_adapter import PsychologyInputAdapter
...
@pytest.mark.asyncio
async def test_ux_grounded_in_live_upstream(request_factory):
    # Phase 3: seed a live knowledge corpus.
    kenv = build_knowledge()
    await _author(
        kenv, KnowledgeCategory.CONVERSION_OPTIMIZATION,
        "Checkout simplicity lifts completion",
        "Reducing checkout fields and friction and honouring accessibility lifts conversion.",
    )
    knowledge = KnowledgeAdvisorAdapter(
        KnowledgeQueryService(kenv.repository, InMemoryKnowledgeSearchPort(kenv.storage))
    )

    # Phases 7, 8, 9: produce live upstream reports.
    strategy_facade, strategy_id = await _produce_strategy()
    brand_facade, brand_id = await _produce_brand()
    psychology_facade, psychology_id = await _produce_psychology()

    # Phase 10: build the UX strategy over the real upstream adapters.
    env = build_ux(
        psychology=PsychologyInputAdapter(psychology_facade, psychology_id),
    ...
    # The strategy must cite the live psychology, brand, business strategy, and knowledge.
    rid = UXReportId.from_string(view.report_id)
    provenances: set[str] = set()
    for kind_name in ("decision", "navigation", "content_hierarchy", "trust_hierarchy", "interaction"):
        for node in view.graphs[kind_name]["nodes"]:
            trace = await env.facade.explain(rid, GraphKind(kind_name), UXNodeId.from_string(node["id"]))
            provenances.update(e["provenance"] for e in trace.evidence)
    # Laws also cite knowledge / upstream evidence.
