"""The Evidence model — the provenance backbone of the UX Strategy Engine.

Every UX decision the engine makes is grounded in :class:`UXEvidence`: a normalised,
cited fact carrying its origin (a :class:`ProvenanceKind` and the id it has in the source
engine — a Psychology finding id, a Brand-Strategy decision id, a Business-Strategy
decision id, a Knowledge lineage id, …). A :class:`Citation` is a lightweight reference
*into* the :class:`EvidenceGraph` used everywhere a decision needs to point at its
support.

This is the anti-hallucination contract, made structural: the report aggregate rejects
any page strategy, journey stage, strategy, analysis, law application, or graph node that
references evidence absent from this graph. No citation ⇒ no UX decision. Everything is
evidence-backed.
...
from ux.domain.shared.value_objects import Confidence, ProvenanceKind, Tag
...
class EvidenceNotFoundError(DesignDirectorError):
    ...
    code = "ux_evidence_not_found"
    http_status = 404


@dataclass(frozen=True, slots=True)
class UXEvidence:
    """One normalised, cited fact underpinning the UX strategy.

    Attributes:
        id: Evidence identity within this report.
        provenance: Which upstream engine (or future source) it came from.
        external_ref: The identity it carries in that source system (a Psychology finding
            id, a Brand-Strategy decision id, …) — the audit anchor back to origin.
        claim: The crisp statement the UX strategy relies on.
    ...
    def __post_init__(self) -> None:
        if not self.claim or not self.claim.strip():
            raise InvalidEvidenceError("UXEvidence.claim must be non-empty.")
        if not self.external_ref or not self.external_ref.strip():
            raise InvalidEvidenceError(
                "UXEvidence.external_ref must be non-empty (audit anchor)."
            )
...
@dataclass(frozen=True, slots=True)
class Citation:
    """A reference from a UX decision to the evidence that supports it.
...
class EvidenceGraph:
    ...
    items: Mapping[UXEvidenceId, UXEvidence] = field(
    ...
    @classmethod
    def of(cls, evidence: Iterable[UXEvidence]) -> EvidenceGraph:
        """Build a graph from evidence.

        Raises:
            InvalidEvidenceError: If two items share an id.
        """
        mapping: dict[UXEvidenceId, UXEvidence] = {}
    ...
    def has(self, evidence_id: UXEvidenceId) -> bool:
        return evidence_id in self.items

    def get(self, evidence_id: UXEvidenceId) -> UXEvidence:
    ...
    def by_provenance(self, provenance: ProvenanceKind) -> tuple[UXEvidence, ...]:
        return tuple(e for e in self.items.values() if e.provenance is provenance)

    def missing(
        self, evidence_ids: Iterable[UXEvidenceId]
    ) -> tuple[UXEvidenceId, ...]:
