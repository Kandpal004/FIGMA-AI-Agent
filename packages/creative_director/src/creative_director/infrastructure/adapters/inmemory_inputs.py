...
and is tested with no upstream engines wired. They are the honest default for local running
and tests (a stand-in for the real Phase 3–12 adapters) and demonstrate the port contracts in
the simplest possible form. A null adapter returns nothing.
"""
...
class InMemoryWireframeInput(_FixedSignals):
    """A :class:`WireframeInputPort` returning fixed wireframe signals."""

    async def gather(self, project: ProjectContext) -> Sequence[RawSignal]:
        return self._signals


class InMemoryIAInput(_FixedSignals):
    """An :class:`IAInputPort` returning fixed information-architecture signals."""

    async def gather(self, project: ProjectContext) -> Sequence[RawSignal]:
        return self._signals


class InMemoryUXInput(_FixedSignals):
    """A :class:`UXInputPort` returning fixed UX signals."""
